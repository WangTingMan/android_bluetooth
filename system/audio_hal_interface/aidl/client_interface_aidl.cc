...
std::vector<AudioCapabilities> BluetoothAudioClientInterface::GetAudioCapabilities(
  ...
  auto aidl_retval =
      provider_factory->m_getProviderCapabilitiesFun(session_type, &capabilities);
  if (!aidl_retval.isOk()) {
...
BluetoothAudioClientInterface::GetProviderInfo(
  ...
  std::optional<IBluetoothAudioProviderFactory::ProviderInfo> provider_info =
      {};
  auto aidl_retval =
      provider_factory->m_getProviderInfoFun(session_type, &provider_info);
...
void BluetoothAudioClientInterface::FetchAudioProvider() {
  ...
  for (int retry_no = 0; retry_no < kFetchAudioProviderRetryNumber; ++retry_no) {
    ...
    capabilities_.clear();
    auto aidl_retval =
            provider_factory->m_getProviderCapabilitiesFun(transport_->GetSessionType(), &capabilities_);
    if (!aidl_retval.isOk()) {
    ...
    aidl_retval = provider_factory->m_openProviderFun(transport_->GetSessionType(), &provider_);
...
bool BluetoothAudioClientInterface::UpdateAudioConfig(const AudioConfiguration& audio_config) {
  ...
  auto aidl_retval = provider_->m_updateAudioConfiguration(audio_config);
  if (!aidl_retval.isOk()) {
...
bool BluetoothAudioClientInterface::SetAllowedLatencyModes(std::vector<LatencyMode> latency_modes) {
  ...
  auto aidl_retval = provider_->m_setLowLatencyModeAllowed(allowed);
  if (!aidl_retval.isOk()) {
    log::warn(
...
int BluetoothAudioClientInterface::StartSession() {
  ...
  auto aidl_retval = provider_->m_startSession(stack_if, transport_->GetAudioConfiguration(),
...
void BluetoothAudioClientInterface::StreamStarted(const BluetoothAudioCtrlAck& ack) {
  ...
  auto aidl_retval = provider_->m_streamStarted(status);
...
void BluetoothAudioClientInterface::StreamSuspended(const BluetoothAudioCtrlAck& ack) {
  ...
  auto status = BluetoothAudioCtrlAckToHalStatus(ack);
  auto aidl_retval = provider_->m_streamSuspended(status);
...
int BluetoothAudioClientInterface::EndSession() {
  ...
  auto aidl_retval = provider_->m_endSession();
...
void BluetoothAudioClientInterface::SetCodecPriority(CodecId codec_id, int32_t priority) {
  log::assert_that(provider_ != nullptr, "assert failed: provider_ != nullptr");
  auto aidl_retval = provider_->m_setCodecPriority(codec_id, priority);
  if (!aidl_retval.isOk()) {
...
BluetoothAudioClientInterface::GetLeAudioAseConfiguration(
  ...
  auto aidl_retval = provider_->m_getLeAudioAseConfiguration(remoteSinkAudioCapabilities,
                                                           remoteSourceAudioCapabilities,
...
BluetoothAudioClientInterface::getLeAudioAseQosConfiguration(
  ...
  auto aidl_retval = provider_->m_getLeAudioAseQosConfiguration(qosRequirement, &qos_configuration);
...
void BluetoothAudioClientInterface::onSinkAseMetadataChanged(
  ...
  auto aidl_retval = provider_->m_onSinkAseMetadataChanged(state, cigId, cisId, metadata);
...
void BluetoothAudioClientInterface::onSourceAseMetadataChanged(
  ...
  auto aidl_retval = provider_->m_onSourceAseMetadataChanged(state, cigId, cisId, metadata);
...
BluetoothAudioClientInterface::getLeAudioBroadcastConfiguration(
  ...
  auto aidl_retval = provider_->m_getLeAudioBroadcastConfiguration(remoteSinkAudioCapabilities,
                                                                 requirement, &setting);
