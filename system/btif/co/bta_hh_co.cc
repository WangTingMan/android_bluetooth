...
#ifdef _MSC_VER
static int uhid_write( int fd, tAclLinkSpec* link_spec, const struct uhid_event* ev, size_t len ) {
#else
...
#ifdef _MSC_VER
    uhid_write( p_uhid->fd, &(p_uhid->link_spec), p_ev, uhid_calc_msg_len( p_ev, p_ev->u.input2.size ) );
#else
...
static int uhid_read_outbound_event(btif_hh_uhid_t* p_uhid) {
  ...
  switch (ev.type) {
    ...
    case UHID_CLOSE:
      ...
      if (!alarm_is_scheduled(p_uhid->ready_disconn_timer)) {
        alarm_set_on_mloop(p_uhid->ready_disconn_timer, BTA_HH_UHID_READY_SHORT_DISCONN_TIMEOUT_MS,
...
static int uhid_read_inbound_event(btif_hh_uhid_t* p_uhid) {
  ...
  switch (ev.type) {
    case BTA_HH_UHID_INBOUND_INPUT_EVT:
      if (p_uhid->ready_for_data) {
#ifdef _MSC_VER
        res = uhid_write( p_uhid->fd, &(p_uhid->link_spec), &ev.uhid, ret - 1 );
#else
...
#ifdef _MSC_VER
    uhid_write( p_uhid->fd, &(p_uhid->link_spec), &ev, uhid_calc_msg_len( &ev, 0 ) );
#else
...
#ifdef _MSC_VER 
  return uhid_write( fd, link_spec, &ev, mlen );
#else
...
#ifdef _MSC_VER
  bta_hh_co_write( p_dev->internal_send_fd, &(p_dev->link_spec), p_rpt, len );
#else
...
#ifdef _MSC_VER
  int result = uhid_write( p_dev->internal_send_fd, &(p_dev->link_spec), &ev, mlen );
#endif
...
void bta_hh_co_get_rpt_rsp(uint8_t dev_handle, uint8_t status, const uint8_t* p_rpt, uint16_t len) {
  ...
  uhid_write( p_dev->internal_send_fd, &(p_dev->link_spec), &ev, len );
#else
