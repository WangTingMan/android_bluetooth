...
LIBBLUETOOTH_API void btif_av_sink_set_audio_focus_state(int focus_state);
LIBBLUETOOTH_API void btif_av_sink_set_audio_track_gain(float gain);
LIBBLUETOOTH_API bt_status_t btif_av_sink_set_active_device(const RawAddress& peer_address);
 ...
 */
LIBBLUETOOTH_API RawAddress btif_av_source_active_peer(void);

/**
 * When the local device is A2DP sink, get the address of the active peer.
 */
LIBBLUETOOTH_API RawAddress btif_av_sink_active_peer(void);
 ...
 */
LIBBLUETOOTH_API void btif_av_stream_start(const A2dpType local_a2dp_type);

/**
 * Start streaming with latency setting.
 */
LIBBLUETOOTH_API void btif_av_stream_start_with_latency(bool use_latency_mode);
 ...
 */
LIBBLUETOOTH_API void btif_av_stream_stop(const RawAddress& peer_address);

/**
 * Suspend streaming.
 */
LIBBLUETOOTH_API void btif_av_stream_suspend(void);

/**
 * Start offload streaming.
 */
LIBBLUETOOTH_API void btif_av_stream_start_offload(void);

/**
 * Check whether ready to start the A2DP stream.
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API bool btif_av_stream_ready(const A2dpType local_a2dp_type);
 ...
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API bool btif_av_stream_started_ready(const A2dpType local_a2dp_type);

/**
 * Check whether there is a connected peer (either Source or Sink)
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API bool btif_av_is_connected(const A2dpType local_a2dp_type);
 ...
 */
LIBBLUETOOTH_API uint8_t btif_av_get_peer_sep(const A2dpType local_a2dp_type);

/**
 * Clear the remote suspended flag for the active peer.
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API void btif_av_clear_remote_suspend_flag(const A2dpType local_a2dp_type);
 ...
 */
LIBBLUETOOTH_API bool btif_av_peer_prefers_mandatory_codec(const RawAddress& peer_address,
 ...
 */
LIBBLUETOOTH_API bt_status_t btif_av_source_execute_service(bool enable);
 ...
 * @return BT_STATUS_SUCCESS on success, BT_STATUS_FAIL otherwise
 */
LIBBLUETOOTH_API bt_status_t btif_av_sink_execute_service(bool enable);
 ...
 * information
 */
LIBBLUETOOTH_API void btif_debug_av_dump(int fd);
 ...
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API void btif_av_set_audio_delay(const RawAddress& peer_address, uint16_t delay,
                             const A2dpType local_a2dp_type);
 ...
 * @param local_a2dp_type type of local a2dp profile.
 */
LIBBLUETOOTH_API uint16_t btif_av_get_audio_delay(const A2dpType local_a2dp_type);

/**
 *  check A2DP offload support enabled
 *  @param  none
 */
LIBBLUETOOTH_API bool btif_av_is_a2dp_offload_enabled(void);

/**
 *  check A2DP offload enabled and running
 *  @param  none
 */
LIBBLUETOOTH_API bool btif_av_is_a2dp_offload_running(void);
 ...
 *
 */
LIBBLUETOOTH_API bool btif_av_is_peer_silenced(const RawAddress& peer_address);
 ...
 */
LIBBLUETOOTH_API void btif_av_set_dynamic_audio_buffer_size(uint8_t dynamic_audio_buffer_size);
 ...
 */
LIBBLUETOOTH_API void btif_av_set_low_latency(bool is_low_latency);
