...
#ifdef _MSC_VER
static void toggle_os_keylockstates(int fd, tAclLinkSpec* link, int changedlockstates) {
#else
...
#ifdef _MSC_VER
  bta_hh_co_write(fd, link, hidreport, sizeof(hidreport));
#else
  ...
  for (unsigned int i = 0; i < sizeof(hid_kb_numlock_on_list) / sizeof(tHID_KB_LIST); i++) {
    ...
    if (dscp_info.vendor_id == kb.version_id && dscp_info.product_id == kb.product_id) {
      ...
      if (keylockstates) {
        ...
        usleep(200000);
#ifdef _MSC_VER
        toggle_os_keylockstates( p_dev->internal_send_fd, &(p_dev->link_spec), keylockstates );
#else
