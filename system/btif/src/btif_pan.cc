...
{
  do_in_main_thread( base::Bind( route_ip_packet_to_remote_with_type, id, a_protocol, p, a_size ) );
}
#endif
...
int btpan_tap_send(int tap_fd, const RawAddress& src, const RawAddress& dst, uint16_t proto,
  ...
  else if (ethernet_packet_type == ETH_P_ARP)
  {
    ...
    status = send_arp_packet_virtual_net( tap_fd, (const char*)local_address.address.data(),
