...
DynamicChannelImpl::DynamicChannelImpl(Psm psm, Cid cid, Cid remote_cid, l2cap::internal::ILink* link,
  ...
  fmt::format_to( std::back_insert_iterator( base_name ), "Local CID:{}", cid );
  std::string name{ base_name };
  name.append( "DynamicChannelImpl::DynamicChannelImpl: up queue name" );
  channel_queue_.SetUpQueueName( name );
  name.clear();
  name.append( base_name );
  name.append( "DynamicChannelImpl::DynamicChannelImpl: down queue name" );
